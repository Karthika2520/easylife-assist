  ...
  head: () => ({
    meta: [
      { title: "Call Family — Contact someone you trust | EASYLIFE" },
      {
        name: "description",
        content:
          "Call a trusted family member or your doctor with one large button. Names, photos and relationships are always shown clearly.",
      },
      ...
      <Page>
        <div className="card-surface flex flex-col items-center gap-6 p-8 text-center">
          ...
          <button
            ...
            className="tap-target min-h-[4rem] w-full max-w-md gap-3 rounded-xl bg-primary px-6 text-action font-semibold text-primary-foreground hover:opacity-90"
          >
        ...
        {familyMembers.map((member) => (
          <li key={member.id} className="card-surface flex flex-col gap-4 p-6">
            <div className="flex min-w-0 items-center gap-4">
              <span
                aria-hidden="true"
                className="grid h-16 w-16 shrink-0 place-items-center rounded-full bg-primary-soft text-h2 font-bold text-primary"
              >
                {member.initials}
              </span>
